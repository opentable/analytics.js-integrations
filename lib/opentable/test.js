...
describe('Opentable', function(){
  var Opentable = plugin;
  var opentable;
  var analytics;
  var options = {
    apiKey: 'oq0vdlg7yi'
  };
  var HOST = 'analytics-collector.pp-uswest2.otenv.com';
  ...
  beforeEach(function(){
    ...
    opentable = new Opentable(options);
    analytics.use(plugin);
    analytics.use(tester);
    analytics.add(opentable);
  ...
  it('should have the right settings', function(){
    analytics.compare(Opentable, integration('Opentable')
      .option('apiKey', ''));
  });
  ...
  it('should always be turned on', function(done){
    ...
    ajs.use(Opentable);
    ajs.initialize({ opentable: options });
    ajs.ready(function(){
      var opentable = ajs._integrations.opentable;
      opentable.ontrack = spy();
      ajs.track('event', {}, { All: false });
      assert(opentable.ontrack.called);
      done();
    });
  });

  describe('Opentable.storage()', function(){
    it('should return cookie() when the protocol isnt file://', function(){
      analytics.assert(Opentable.storage(), cookie);
    });

    it('should return store() when the protocol is file://', function(){
      analytics.assert(Opentable.storage(), cookie);
      protocol('file:');
      analytics.assert(Opentable.storage(), store);
    });
  ...
  describe('before loading', function(){
    ...
    describe('#normalize', function(){
      ...
      beforeEach(function(){
        opentable.cookie('s:context.referrer', null);
      ...
      it('should add .writeKey', function(){
        opentable.normalize(object);
        analytics.assert(opentable.options.apiKey == object.writeKey);
      });
      ...
      it('should add .referrer.id and .referrer.type from cookie', function(){
        opentable.cookie('s:context.referrer', '{"id":"baz","type":"millennial-media"}');
        plugin.global = { navigator: {}, location: {} };
      ...
      it('should add .referrer.id and .referrer.type from cookie when no query is given', function(){
        opentable.cookie('s:context.referrer', '{"id":"medium","type":"millennial-media"}');
        plugin.global = { navigator: {}, location: {} };
  ...
  describe('after loading', function(){
    ...
    describe('#page', function(){
      ...
      it('should send section, name and properties', function(){
        ...
        analytics.assert(true == args[1].options.opt);
    ...
    describe('#send', function(){
      beforeEach(function(){
        analytics.spy(opentable, 'session');
      });

      it('should use http: protocol when http:', function(done){
        protocol('http:');
        opentable.send('/i', { userId: 'id' }, function(err, res){
          if (err) return done(err);
          assert.equal('http://'+ HOST +'/v1/i', res.url);
          done();
        });
      });
      ...
      it('should use https: protocol when https:', function(done){
        ...
        segment.send('/i', { userId: 'id' }, function(err, res){
          if (err) return done(err);
          assert.equal('https://'+ HOST +'/v1/i', res.url);
          done();
        });
      });
